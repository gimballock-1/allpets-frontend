...
- **Single `/` rule, app-auth-only (3.6):** Payload serves **`/admin`** (and `/api`) on the **same origin** behind the Next.js site — **no second `/admin` path rule, no auth middleware** at ingress. The same-host `/admin` routing is coordinated with 5.13/3.4/3.6 so the frontend Ingress and Payload don't collide (the site app must pass `/admin`/`/api` through to Payload or the two are reconciled on the same host per 5.13).
