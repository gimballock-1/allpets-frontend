...
>
> **What is NOT used (recorded so a grep finds only disclaimers):** **no Payload / no CMS** — marketing content is **file-based** (typed TS + MDX in this repo), there is **no content API, no `PAYLOAD_INTERNAL_URL`, no same-origin `/admin`, and no same-origin `/api` content seam**; **no Payload-proxied MinIO media for the marketing site** (marketing images are local `public/` assets; MinIO is reserved for the backend's future pet-profile media — HLD *Phase-2 direction*); **no Cloudflare Tunnel/Access** — but the original "no Cloudflare proxy" stance is **superseded** (2026-06 domain pivot + 14.2 live finding): hosts live on `*.skpodduturi.dev` behind Cloudflare DNS and the public hosts are **orange-cloud proxied**, so `CF-Connecting-IP` is the only header carrying the real visitor IP end-to-end (k3s klipper ServiceLB SNATs external traffic to the node CNI address before Traefik, so Traefik's `X-Forwarded-For` never does — see §9 in-cluster note and `src/app/api/contact/route.ts`); **no HTTP-01** (solver is DNS-01); **no NodePort** (Traefik Ingress on :80/:443); **no sealed-secrets/SOPS/external-secrets** in phase 1; **no GitOps** (push-based CD); the site **never** opens a Postgres/MinIO connection (no `allpets-frontend → allpets-database` path — HLD *Three-namespace topology* / *Architecture Decisions*).
...
- **Backend port 3000** matches the committed NetworkPolicy contract (HLD *Three-namespace topology*, `allow-traefik-ingress` frontend). The site has **no data-tier path**. The original "no in-cluster backend path" stance is **superseded** (14.2, backend #218): the server-side proxy hop now targets the **in-cluster Service `http://allpets-api.allpets-backend.svc.cluster.local:8080`** (deploy/k8s/deployment.yaml `API_BASE`), NOT the public host — the public host is Cloudflare-proxied, so a server-side fetch through it reaches Spring from a Cloudflare edge address and per-IP rate limiting collapses into one shared bucket. The cross-namespace `allpets-frontend → allpets-backend:8080` ingress allow ships in the backend repo's NetworkPolicies.
